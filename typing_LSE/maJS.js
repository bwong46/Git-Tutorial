...
const tazaDisplay = document.querySelector('.taza_avg');
...
function checkMatch() {
    ...
    if(window.event.keyCode == 13 &&  score != 6) {
        ...
        strDisplay.innerText = strWords[randomIndex]
        strWords[randomIndex] = strW[randomIndex]

        // while(strInput.value.length === strDisplay.innerText.length) {
        //     taza_count = 0;

        //     timestamp = new Date().getTime();  // 현재 시간
        //     taza_count = gettext(user);
            
        //     time = elapsedTime - timestamp;

        //     if(taza_count == 0) break; //count가 0이면 정지

        //     n = parseFloat(60) / (time / 1000);
        //     tazaDisplay = parseInt(n) * taza_count; 
